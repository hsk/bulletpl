...
disp_bullet(Bs3) :-
  get_time(Time),nb_getval(time,OTime),W is (1/60-(Time-OTime)),sleep(W),
  Time2 is Time+max(-1/60,W),nb_setval(time,Time2), 
...
:- initWindow,initShip,get_time(Time),nb_setval(time,Time),main.
:- halt.
