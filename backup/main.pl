...
% enemy
  ...
  moveEnemy(Enemy) :- M=Enemy.move,call(M,Enemy).
  moveEnemy(Enemy) :- writeln(error:moveEnemy:Enemy),throw(error).
  moveBodyEnemy(E,E_,Rad2) :-
    bb_get(ship,Ship),
  ...
  waitEnemy(W,E,Next) :-
    E2 = E.put(time,W).put(move,moveWaitEnemy).put(nextMove,Next),
    bb_update(enemies,Enemies,[E2|Enemies]).
  moveWaitEnemy(E) :-
    moveBodyEnemy(E,E1,_),
    Time is E1.time - 1,
    E2=E1.put(time,Time),
    (Time =< 0 -> bb_update(enemies,Enemies,[E2.put(move,E2.nextMove)|Enemies])
    ; bb_update(enemies,Enemies,[E2|Enemies])).
  move1Enemy(E) :-
    moveBodyEnemy(E,E_,R),
    random(0.0,1.0,R1),
    (R1 > 0.8 -> !,
      addBullet(E_.x,E_.y,R),
      random(0.0,1.0,R2),
      (R2 > 0.8 -> !,
        waitEnemy(30,E_.put(r2,0.0),move2Enemy)
      ;!,bb_update(enemies,Enemies,[E_|Enemies]),!)
    ;bb_update(enemies,Enemies,[E_|Enemies]),!).
  move2Enemy(E):-
    moveBodyEnemy(E,E_,R),
    random(1,6,RR),
    R1 is R + E.r2,
    addBulletN(E_.x,E_.y,R1,RR,0.1),
    R2 is float(E_.r2+RR*0.1),
    E1 = E_.put(r2,R2),
    random(0.0,1.0,R3),
    (R3 < 0.03 ->
      waitEnemy(60,E1,move1Enemy)
    ;bb_update(enemies,Enemies,[E1|Enemies])).
  addEnemy :-
    bb_get(rank,Rank),
  ...
  moveEnemies :-
    ...
    %bb_get(enemies,Es),!,writeln(enemies:Es),!,
    addEnemy,!.
  drawEnemies :-
    bb_get(enemies,Enemies),
