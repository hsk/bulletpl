...
changeDirection(dirSeq(D),T) :- nb_getval(bullet,B),D_ is2 D,nb_linkval(bullet,B.put(cd,(D_,T))).
changeDirection(dirAbs(D),T) :- nb_getval(bullet,B),D_ is(D-B.d)/T,nb_linkval(bullet,B.put(cd,(D_,T))).
changeDirection(dirRel(D),T) :- nb_getval(bullet,B),T_ is2 floor(T),D_ is2 D/T_,nb_linkval(bullet,B.put(cd,(D_,T_))).
changeDirection(dirAim(D),T) :- nb_getval(bullet,B),nb_getval(ship,Ship),d(B,dirAim(D),Ship,D1),D2 is2 (D1-B.d),T_ is2 T,
                                D_ is2 (((floor(D2) + 180)mod 360)-180)/T_,nb_linkval(bullet,B.put(cd,(D_,T))).
changeSpeed(spdSeq(S),T) :- nb_getval(bullet,B),S_ is2 S*1.68,nb_linkval(bullet,B.put(cs,(S_,T))).
changeSpeed(spdAbs(S),T) :- nb_getval(bullet,B),S_ is(S*1.68-B.s)/T,nb_linkval(bullet,B.put(cs,(S_,T))).
changeSpeed(spdRel(S),T) :- nb_getval(bullet,B),S_ is2 S*1.68/T, nb_linkval(bullet,B.put([cs:(S_,T),isAim:0])).
...
move([]) :- !.
move(Bs) :-
  nb_getval(move_cnt,N),N1 is2 N+1,nb_linkval(move_cnt,N1),!,
  (N1 > 600 -> true;
  nb_linkval(bullets,''),
...
setDefs(Ds) :-
  ...
  maplist(setDef,Ds).
setRank(N) :- V is2 N, nb_linkval(rank,V).
rankUp :- nb_getval(rank,N),N1 is2 N+1,nb_linkval(rank,N1).
:- use_module(syntax,[]).
setColor(K) :- nb_getval(color_cnt,C),C1 is2 C+1,nb_linkval(color_cnt,C1),asserta(color(K,C)).
