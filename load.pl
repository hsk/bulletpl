...
expr(E1*E2,E1_*E2_) :- expr(E1,E1_),expr(E2,E2_).
expr(E1 div E2,E1_ div E2_) :- expr(E1,E1_),expr(E2,E2_).
expr(E1 mod E2,E1_ mod E2_) :- expr(E1,E1_),expr(E2,E2_).
expr(A,R) :- atom(A),
  re_replace('\\$'/g,' $',A,A1),
  re_replace('%'/g,' mod ',A1,A2),
  re_replace('/'/g,' div ',A2,A3),
  re_replace('-'/g,' - ',A3,A4),
  atom_string(A5,A4),
  atom_to_term(A5,T,[]),!,
  expr(T,R).
...
speed([element(speed,[type=Type],[E])|A],(some(R),A)) :- !,speedType(Type,E_,R),expr(E,E_).
speed([element(speed,[],[E])|A],(some(R),A)) :- expr(E,E_),speedType(none,E_,R).
speed(A,(none,A)).

direction([element(direction,[type=absolute],[E])|A],(some(dirAbs(E_)),A)) :- expr(E,E_).
direction([element(direction,[type=sequence],[E])|A],(some(dirSeq(E_)),A)) :- expr(E,E_).
direction([element(direction,[type=aim],[E])|A],(some(dirAim(E_)),A)) :- expr(E,E_).
direction([element(direction,[type=relative],[E])|A],(some(dirRel(E_)),A)) :- expr(E,E_).
direction(A,(none,A)).

horizontal([element(horizontal,[type=T],[E])|A],some(R),A) :- expr(E,E_),speedType(T,E_,R).
horizontal([element(horizontal,_,[E])|A],some(R),A) :- expr(E,E_),speedType(none,E_,R).
horizontal(A,none,A).

vertical([element(vertical,[type=T],[E])|A],some(R),A) :- expr(E,E_),speedType(T,E_,R).
vertical([element(vertical,_,[E])|A],some(R),A) :- expr(E,E_),speedType(none,E_,R).
vertical(A,none,A).
...
action1(element(changeSpeed,[],A),changeSpeed(Sp,E_)) :-
  speed(A,(some(Sp),[element(term,[],[E])])),expr(E,E_).
action1(element(changeDirection,[],A),changeDirection(D_,E_)) :- !,
  direction(A,(some(D_),[element(term,[],[E])])),expr(E,E_).
...
elem(element(action,[label=Label],A),eaction(Label,A_)) :- !,
  atom(Label),maplist(action1,A,A_).
elem(element(bullet,[label=Label],A),ebullet(Label,A_)) :- !,
  atom(Label),bullet1(element(bullet,[],A),A_).
elem(element(fire,[label=Label],A),efire(Label,A_)) :-
