...
evalto(A,B) :- catch(A is B,_,false),!.
...
cont(B,_) :- (B.x < 0; B.y < 0; B.x > 300; B.y > 400),
             asserta(bullet(B)),shift(1). % 画面外で消える
cont(B,N) :- asserta(bullet(B)),shift(0),N1 is N - 1, wait(N1).
...
changeSpeed(spdAbs(S),T) :- retract(bullet(B)),S_ is(S*1.6-B.s)/T,asserta(bullet(B.put(cs,(S_,T)))).
changeSpeed(spdRel(S),T) :- retract(bullet(B)),S_ is S*1.6/T, asserta(bullet(B.put([cs:(S_,T),isAim:0]))).
accelspd(spdSeq(S),_,_,S_) :- S_ is S*1.6.
accelspd(spdAbs(S),M,T,S_) :- S_ is (S*1.6-M)/T.
accelspd(spdRel(S),_,T,S_) :- S_ is S*1.6 / T.
accelspd(none,_,_,0).
